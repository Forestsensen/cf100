...
export async function GET(request: Request) {
  ...
  const cacheKey = clientRange
    ? null
    : new Request(decodedUrl, { method: 'GET' });
  ...
  // 透传客户端条件请求头，支持上游 304（省流量）
  const clientInm = request.headers.get('if-none-match');
  const clientIms = request.headers.get('if-modified-since');
  if (clientInm) upstreamHeaders['If-None-Match'] = clientInm;
  if (clientIms) upstreamHeaders['If-Modified-Since'] = clientIms;
  ...
  try {
    ...
    return proxyErrorResponse(error, 'segment');
  } finally {
    clearTimeout(timeoutId);
  }
  // 上游返回 304 → 直接透传（无 body）
